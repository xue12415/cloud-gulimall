    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.attrId) {
            this.$http({
              url: this.$http.adornUrl(`/product/pmsattr/info/${this.dataForm.attrId}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.valueType = data.pmsAttr.valueType
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/product/pmsattr/${!this.dataForm.attrId ? 'save' : 'update'}`),
              method: 'post',
